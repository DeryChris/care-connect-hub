...
const KnowledgeArticlePage = () => {
  ...
  const perms = hasContentPermission(user, 'update', 'knowledge', article.author_id);
                ...
                <div className="flex gap-2 shrink-0">
                  ...
                  {transitions.includes('approved') && artStatus === 'review' && canApprove && (
                    <Button
                      ...
                      onClick={() => updateStatus.mutate({ id: article.id, status: 'approved' })}
                    >
                  ...
                  {related.map(rel => (
                    <Link key={rel.id} to={`/knowledge/${rel.id}`} className="block p-2 rounded-lg hover:bg-muted transition-colors">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge className={STATUS_COLORS[rel.status as keyof typeof STATUS_COLORS] || ''} style={{ fontSize: '10px', padding: '1px 6px' }}>
                          {STATUS_LABELS[rel.status as keyof typeof STATUS_LABELS] || rel.status}
                        </Badge>
