...
import {
  Search, Receipt, DollarSign, Clock, AlertCircle, CheckCircle,
} from 'lucide-react';
...
// Simple inline currency formatter — avoids the SettingsContext import issue
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: 'Total Revenue',    value: fmt(summary?.totalRevenue  ?? 0), icon: DollarSign,  color: 'text-success',          bg: 'bg-success/10' },
          { label: 'Pending Amount',   value: fmt(summary?.pendingAmount ?? 0), icon: Clock,       color: 'text-warning',          bg: 'bg-warning/10' },
                ...
                : invoices.map(inv => {
                    ...
                    return (
                      <TableRow key={inv.id}>
                        ...
                        <TableCell>
                          <Badge variant="outline" className="capitalize">{inv.billing_type}</Badge>
                        </TableCell>
                        <TableCell className="font-medium">{fmt(inv.total)}</TableCell>
                        <TableCell className="text-success">{fmt(inv.amount_paid)}</TableCell>
                        ...
                        <TableCell className="text-right">
                          {(inv.status === 'pending' || inv.status === 'partial') && (
                            <Button
                              ...
                              onClick={() => { setPaymentDialog(inv.id); setPaymentAmount(''); }}
                            >
                              Record Payment
                            </Button>
                          )}
              ...
              onClick={() => {
                if (paymentDialog && paymentAmount) {
                  recordPayment.mutate(
                    { id: paymentDialog, amount: parseFloat(paymentAmount) },
